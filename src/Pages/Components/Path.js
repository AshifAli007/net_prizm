...
const Path = ({ map, phone }) => {
  ...
  const calculateRoute = async () => {
    ...
    let { data: results } = await axios.get(
      `https://api.mapbox.com/directions/v5/mapbox/${phone.mode}/${origin};${destination}?geometries=geojson&access_token=${process.env.REACT_APP_BOX_API_KEY}`
    );
    ...
    map.loadImage(carImage, (error, image) => {
    ...
    map.addLayer({
      ...
      paint: {
        "line-color": `rgb(${Math.random() * 200 + 50},${
          Math.random() * 200 + 50
        },${Math.random() * 200 + 50})`,
        "line-width": 5,
      },
    ...
    function animate() {
      ...
      point.features[0].properties.currentLatLng =
        route.features[0].geometry.coordinates[counter];
      ue_popup.setHTML(
  ...
  useEffect(() => {
    ...
    destinationDiv.addEventListener("mouseenter", () =>
      destinationMarker.togglePopup()
    );
    destinationDiv.addEventListener("mouseleave", () =>
      destinationMarker.togglePopup()
    );
